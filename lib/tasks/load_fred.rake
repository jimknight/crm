namespace :load_data do
  ...
  task :import_for_fred => :environment do
    require 'area'
    require 'csv'
    require 'roo'
    xlsx = Roo::Spreadsheet.open(Rails.root.to_s + "/vendor/data/FredRiessContacts.xlsx")
    ...
    csv.each do |row|
      ...
      if client_name.present?
        ...
        if new_client.changed?
          new_client.save!
          user = User.find_by_email('friess@mixers.com')
